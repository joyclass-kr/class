...
assert.equal((page.match(/class="column-labels"/g) || []).length, 4);
...
assert.match(app, /catalog\.dataset\.gradeFilter = grade/);
assert.match(styles, /\.topic-list\{display:grid;grid-template-columns:1fr\}/);
assert.match(styles, /\.level-list\{display:grid;grid-template-columns:repeat\(3,minmax\(0,1fr\)\)/);
assert.match(styles, /\.level-entry\[data-level="high"\]\{grid-column:3;grid-template-columns:1fr\}/);
assert.match(styles, /main\{width:min\(1240px/);
assert.match(styles, /\.level-entry span\{font-size:14px/);
assert.match(styles, /\.level-entry\[data-level="middle"\] b,\.level-entry\[data-level="high"\] b\{display:none\}/);
