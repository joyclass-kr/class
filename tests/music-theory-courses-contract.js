...
const harmonyApp = fs.readFileSync(path.join(root, "harmony", "app.js"), "utf8");
const harmonyAudio = fs.readFileSync(path.join(root, "harmony", "piano-engine.js"), "utf8");
const harmonyCss = fs.readFileSync(path.join(root, "harmony", "styles.css"), "utf8");
...
assert.match(harmonyHtml, /id="stageList"/);
assert.match(harmonyHtml, /id="piano"/);
assert.match(harmonyHtml, /piano-engine\.js/);
assert.equal((harmonyApp.match(/\{ id: \d+, title:/g) || []).length, 10, "화성학은 10단계여야 합니다.");
assert.match(harmonyApp, /musicTheoryHarmonyProgressV1/);
