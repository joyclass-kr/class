...
assert.ok(view.reachable.cells.includes("g3_5"), "온실의 실제 문으로 나가는 목적지가 보여야 합니다.");
assert.ok(view.reachable.cells.includes("g5_5"), "주사위 범위 안의 최종 목적지를 한 번에 표시해야 합니다.");
assert.ok(!view.reachable.cells.includes("g6_5"), "주사위 범위를 넘는 목적지는 표시하면 안 됩니다.");
assert.deepEqual(view.reachable.rooms, []);

assert.equal(Clue.move(game, "host", "g5_5").ok, true);
assert.equal(game.stepsRemaining, 0);
assert.equal(game.players[0].cellId, "g5_5");
assert.deepEqual(game.lastMove.path, ["r0", "g3_5", "g4_5", "g5_5"], "서버가 최종 목적지까지의 실제 경로를 저장해야 합니다.");
...
assert.ok(doorView.reachable.rooms.includes(1),"문 타일에서는 그 문에 직접 연결된 방만 표시해야 합니다.");
...
wallGame.diceValues=[0,1];
wallGame.dice=1;
wallGame.stepsRemaining=1;
...
assert.match(html, /최종 목적지를 한 번 선택/, "최종 목적지 한 번 클릭 규칙을 표시해야 합니다.");
