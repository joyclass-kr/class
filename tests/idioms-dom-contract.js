...
const html = fs.readFileSync("learning/literacy-numeracy/classical-chinese-idioms/index.html", "utf8");
const app = fs.readFileSync("learning/literacy-numeracy/classical-chinese-idioms/app.js", "utf8");
...
data.forEach((item) => {
    ...
    assert.ok(
        fs.existsSync(`learning/literacy-numeracy/classical-chinese-idioms/${illustration}`),
        `${item.word} 삽화 파일이 필요합니다: ${illustration}`
    );
});
assert.ok(!/탐험대|뜻만 외우지 말고|이야기 박사|멋진 도전/.test(html + app), "과장된 홍보 문구를 사용하지 않습니다.");
assert.ok(root.includes('href="learning/literacy-numeracy/classical-chinese-idioms/"'), "메인 학습 메뉴에 한자성어 링크가 필요합니다.");
