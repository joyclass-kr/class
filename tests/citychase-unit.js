...
assert.ok(Object.keys(CityChase.BOARD.NODES).length >= 70, "사진 속 여러 갈래 길을 표현할 충분한 보드 칸이 필요합니다.");
...
assert.equal(CityChase.moveStep(game, "thief", "p1").ok, true);
...
captureGame.pawns.find(pawn => pawn.id === "thief-1").position = "p1";
...
assert.equal(CityChase.moveStep(captureGame, "p", "p1").ok, true);
...
assert.match(css, /\.node::after\s*\{[^}]*inset:\s*-9px/s, "작은 보드 칸에도 46px 터치 영역이 필요합니다.");
