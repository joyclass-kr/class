...
for (const file of ["questions.js", "questions-extra.js", "question-deck.js"]) {
    const filePath = path.join(root, "learning", "literacy-numeracy", "spelling", file);
...
assert.equal(completeBankPass.length, questions.length);
assert.equal(new Set(completeBankPass).size, questions.length, "All questions must appear before any question repeats.");
assert.deepEqual(
...
console.log("spelling-question-deck-unit: all unique questions before repeat, storage recovery ok");
