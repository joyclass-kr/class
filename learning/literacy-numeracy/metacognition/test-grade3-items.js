/* 초3 레벨 문항 세트 검증 — node test-grade3-items.js
 * test-metrics.js와 같은 규칙을 16문항 세트에 맞게 적용한다. */
const assert = require("assert");
const { METACOG_ITEMS_G3 } = require("./items-grade3.js");
...
check("문항 id 중복 없음", () => {
  const ids = new Set(METACOG_ITEMS_G3.map((item) => item.id));
  assert.strictEqual(ids.size, METACOG_ITEMS_G3.length);
});

check("kind 구성비: plain 3 이상, trap 6 이상, looksHard 3 이상", () => {
  const count = (kind) => METACOG_ITEMS_G3.filter((item) => item.kind === kind).length;
...
function strategyScore(pick) {
  return METACOG_ITEMS_G3.filter((item, index) => pick(item, index) === item.answer).length;
}
...
check("정답이 혼자 가장 긴 선택지인 문항이 우연 수준이다", () => {
  const soleLongest = METACOG_ITEMS_G3.filter((item) => {
    const lengths = item.choices.map((choice) => choice.length);
    const max = Math.max(...lengths);
