...
function coverPage() {
 ...
 <p>그 안에서 나온 이야기 가운데 여덟 편을 골라 담았습니다.</p>
 </div>
 </div>`;
}
...
const QUIZ = [
    ...
    { q: "이 책을 읽고 난 반응으로 알맞지 않은 것은 무엇인가요?", wide: true, choices: ["셰에라자드가 밤마다 이야기를 제일 재미있어지는 데서 멈춘 것을 보면, 궁금함이 사람을 하루 더 살리기도 하는구나.", "알리바바네 집을 지킨 사람이 밥 짓고 물 긷던 모르지아나였던 것을 보면, 집을 지킨 건 주인이 아니라 부리던 사람이었구나.", "알라딘이 램프가 무엇인지 아내에게 미리 일러 두어 램프를 지켜 낸 것을 보면, 가까운 사람에게 말해 두는 게 지키는 길이구나.", "신드바드가 바다의 노인을 딱해서 업어 주고도 후회하지 않는 것을 보면, 당한 뒤에도 딱하게 여긴 마음까지 물리지는 않는구나."], answer: 2 }
];
