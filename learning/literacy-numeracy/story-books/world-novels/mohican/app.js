...
const QUIZ = [
    ...
    { q: "이 책을 읽고 난 반응으로 알맞지 않은 것은 무엇인가요?", wide: true, choices: ["마과가 왜 그렇게 되었는지 3장에 나오는 것을 보면, 마과만 나쁘다고 하면 앞이 안 보이는 이야기구나.", "매사냥꾼이 백인인데 원주민 사이에서 자란 것을 보면, 양쪽 말을 다 하면서 어느 쪽에도 온전히 못 속한 사람이구나.", "매사냥꾼이 원주민 말을 몰라 늘 통역을 데리고 다닌 것을 보면, 두 세계 사이를 오간 건 말이 아니라 총이었구나.", "아들 웅카스가 먼저 가고 칭가치국이 남은 것을 보면, 제목의 마지막 사람은 한 부족이 끊긴다는 뜻이구나."], answer: 2 }
];
