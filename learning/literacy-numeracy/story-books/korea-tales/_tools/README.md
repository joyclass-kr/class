...
| `artorder.py [책]` | 그림이 제자리에 붙었는지 본다 (차례·빠짐·남음·중복) |
| `imgratio.py [책]` | 그림 파일의 실제 가로세로 비율을 재 본다 |
| `tools-artmatch.mjs [책]` | 그림 지시문과 본문이 어긋나는지 본다 (달·별, 매어 둔 짐승) |
| `tools-hardwords.mjs [책]` | 아이가 모를 말에 풀이가 붙었는지 본다 (처음 나온 자리만) |
