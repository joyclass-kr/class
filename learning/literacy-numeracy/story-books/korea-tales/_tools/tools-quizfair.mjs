...
for (const b of books) {
  ...
  QUIZ.forEach((q, idx) => {
    ...
    /* 「읽고 난 반응」은 보기가 다 한 문장이라 맺음말이 눈에 띈다.
       셋이 똑같이 "…구나"인데 틀린 보기만 "…을까"로 끝나면, 뜻을 안 읽고
       말투만 보고도 고를 수 있다.

       그렇다고 맺음말을 맞추라는 말은 아니다. 넷이 저마다 다르면 튀는 것이
       없으니 아무 표도 나지 않는다. 나쁜 것은 "셋은 한 꼴, 하나만 딴 꼴"
       이 경우뿐이다. 문장마다 자연스러운 맺음말을 쓰되 이것만 피한다. */
    if (q.wide) {
      const 맺음 = c => c.trim().replace(/[.]$/, '').slice(-2);
      const 내꼴 = 맺음(ans);
      const 남들 = others.map(맺음);
      const 남들이한꼴 = new Set(남들).size === 1;
      if (남들이한꼴 && 남들[0] !== 내꼴)
        why.push('셋은 한 꼴인데 틀린 보기만 딴 꼴 · 「' + 내꼴 + '」 vs 「' + 남들[0] + '」');
    }
