...
test("6소수③은 두 반올림 구역과 몫·나머지 구역을 3문제씩 만든다", () => {
  ...
  assert.equal(problems.filter((problem) => problem.section === "remainder").length, 3);
});
...
test("두 소수의 나눗셈을 지정한 자리에서 정확하게 반올림한다", () => {
  ...
  for (let seed = 1; seed <= 100; seed += 1) {
    for (const problem of createGradeSixDecimalThreeSet(seed).problems.slice(0, 6)) {
      assert.equal(
        problem.answer,
        roundDecimalDivision(
          problem.leftUnits,
...
test("몫과 나머지는 원래 나눗셈을 정확히 복원하고 나머지가 나누는 수보다 작다", () => {
  ...
  for (let seed = 1; seed <= 100; seed += 1) {
    for (const problem of createGradeSixDecimalThreeSet(seed).problems.slice(6)) {
