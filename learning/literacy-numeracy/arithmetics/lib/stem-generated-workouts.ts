...
function choices(id: string, answer: string, alternatives: string[]) {
  ...
  if (values.length < 4) throw new Error(`${id}: 실제 오답 후보가 3개보다 적습니다.`);
...
export function createPartialDerivativeProblems(seed: number): GeometryChoiceItem[] {
  ...
  return [
    ...
    item("pd3", "편미분값", `f(x,y)=x^2y+${a}y^2,\\quad f_x(${b},1)=?`, `${2 * b}`, [`${b * b}`, `${2 * b + 2 * a}`, `${b * b + a}`, `${2 * b + a}`, `${b + 2 * a}`, `${b * b + 2 * a}`, `${2 * b - a}`]),
...
export function createEulerFormulaProblems(seed: number): GeometryChoiceItem[] {
  ...
  return [
    ...
    item("eu5", "복소수의 곱", `${r}e^{i\\pi/${k + 2}}\\cdot 2e^{i\\pi/${k + 3}}=?`, `${2 * r}e^{i(\\pi/${k + 2}+\\pi/${k + 3})}`, [`${2 * r}e^{i(\\pi/${k + 2}-\\pi/${k + 3})}`, `${r + 2}e^{i(\\pi/${k + 2}+\\pi/${k + 3})}`, `${2 * r}e^{i\\pi/${(k + 2) * (k + 3)}}`, `${r}e^{i(\\pi/${k + 2}+\\pi/${k + 3})}`, `${2 * r}e^{i(2\\pi/${k + 2}+\\pi/${k + 3})}`]),
...
export function createMatrixProblems(seed: number): GeometryChoiceItem[] {
  ...
  return [
    ...
    item("mx2", "행렬의 곱", `\\begin{pmatrix}${a}&${b}\\\\${c}&${d}\\end{pmatrix}\\begin{pmatrix}1\\\\2\\end{pmatrix}=?`, `\\begin{pmatrix}${a + 2 * b}\\\\${c + 2 * d}\\end{pmatrix}`, [`\\begin{pmatrix}${a + b}\\\\${2 * c + d}\\end{pmatrix}`, `\\begin{pmatrix}${a + 2 * c}\\\\${b + 2 * d}\\end{pmatrix}`, `\\begin{pmatrix}${a * b}\\\\${2 * c * d}\\end{pmatrix}`, `\\begin{pmatrix}${a + 2 * d}\\\\${b + 2 * c}\\end{pmatrix}`, `\\begin{pmatrix}${2 * a + b}\\\\${2 * c + d}\\end{pmatrix}`, `\\begin{pmatrix}${2 * a + 2 * b}\\\\${2 * c + 2 * d}\\end{pmatrix}`, `\\begin{pmatrix}${a - b}\\\\${c - d}\\end{pmatrix}`]),
    item("mx3", "행렬식", `\\det\\begin{pmatrix}${a}&${b}\\\\${c}&${d}\\end{pmatrix}=?`, `${det}`, [`${a * d + b * c}`, `${a * c - b * d}`, `${a + d - b - c}`, `${b * c - a * d}`, `${a * d}`, `${b * c}`, `${a + d}`, `${a * b * c * d}`, `${a + b + c + d}`, `${a * d - b - c}`, `${a * d + b + c}`, `${a * b + c * d}`]),
...
export function createIntegralApplicationProblems(seed: number): GeometryChoiceItem[] {
  ...
  return [
    ...
    item("ia2", "회전체의 부피", `y=${a}x,\\quad 0\\le x\\le${b}\\text{를 }x\\text{축 둘레로 회전},\\quad V=?`, `\\frac{${a * a * b ** 3}\\pi}{3}`, [`${a * b * b}\\pi`, `\\frac{${a * b ** 3}\\pi}{3}`, `${a * a * b ** 3}\\pi`, `\\frac{${a * a * b ** 3}\\pi}{2}`, `${a * a * b * b}\\pi`]),
    ...
    item("ia5", "타원의 넓이", `\\frac{x^2}{${a * a}}+\\frac{y^2}{${b * b}}=1,\\quad A=?`, `${a * b}\\pi`, [`${a + b}\\pi`, `${a * a + b * b}\\pi`, `2${a * b}\\pi`, `\\frac{${a * b}\\pi}{2}`, `${a * a * b * b}\\pi`, `2(${a + b})\\pi`]),
