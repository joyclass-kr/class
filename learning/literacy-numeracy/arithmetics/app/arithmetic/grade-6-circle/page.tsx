...
function CircleCompositeDiagram({ problem, instance }: { problem: CircleProblem; instance: "worksheet" | "answers" }) {
  ...
  if (problem.kind === "semicircle-circle-hole") return <svg viewBox="0 0 260 180" role="img" aria-label="반원에서 안쪽 원을 뺀 색칠한 부분"><path d="M35 155 A95 95 0 0 1 225 155 L35 155 Z" {...common} /><circle cx="130" cy="107.5" r="47.5" {...white} /><circle cx="130" cy="107.5" r="2.2" fill="#17233c" /><line x1="130" y1="107.5" x2="177.5" y2="107.5" className="circle-svg-measure" />{label(154, 100, d.innerRadius + "cm")}</svg>;
...
export default function GradeSixCirclePage() {
  ...
  return <main className="counting-page multiplication-page"><div className="counting-toolbar"><a className="counting-back" href="/arithmetic" aria-label="연산 목록으로 돌아가기">←</a><div className="counting-progress"><strong>{correct}<small>/6 정답</small></strong></div><div className="toolbar"><button className="button secondary" onClick={fresh}>새 문제</button><button className="button ghost" onClick={reset}>다시 풀기</button><div className="print-control"><button className="button ghost print-button" onClick={() => setPrintOpen(!printOpen)}>인쇄</button>{printOpen && <div className="print-menu"><button onClick={() => print("worksheet")}>문제지만 인쇄</button><button onClick={() => print("answers")}>답지만 인쇄</button><button onClick={() => print("both")}>문제지+답지 인쇄</button></div>}</div><button className="button primary" onClick={check}>전체 채점</button></div></div><div className="a4-stage counting-a4-stage worksheet-stage" style={{ width: 794 * scale, height: 1123 * scale }} aria-label="A4 6학년 원의 둘레와 넓이 문제지">{sheet(false)}</div><div className="a4-stage counting-a4-stage answer-stage" style={{ width: 794 * scale, height: 1123 * scale }} aria-label="A4 6학년 원의 둘레와 넓이 전체 답지">{sheet(true)}</div></main>;
}
