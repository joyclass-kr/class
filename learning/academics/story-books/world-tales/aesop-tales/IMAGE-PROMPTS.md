...
> 이미 만드신 결승선 그림은 **3번째 자리**가 맞아서 `story-01-race-3.webp`로 파일명을 바꿔 두었어요.
> - `story-01-race.webp` — 이미 있음. 다만 예전 4:3 비율이라 **가로 2:1로 다시 한 번** 뽑아 주세요.
> - `story-01-race-2.webp` — **새로 필요합니다** (출발 장면).
> - `story-01-race-3.webp` — 이미 있음, 그대로 두시면 됩니다.
> 표지(`cover.webp`)와 마무리(`end.webp`)도 그대로 두세요.

권장 크기: **본문 그림은 가로 2 : 세로 1 비율**(가로로 아주 길쭉한 모양)로 만들어 주세요, PNG. 펼침면 전체 폭을 채우는 자리라 이렇게 길어야 잘리지 않아요. **표지(`cover.webp`)는 세로 2 : 3 비율**, 마무리(`end.webp`)는 가로 3 : 세로 2 정도면 됩니다.
