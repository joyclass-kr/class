/**
 * 태양계 탐험 (Solar System Explorer) Three.js & HTML5 Canvas Engine
...
(function () {
    ...
    function boot() {
        ...
        // 1. ALWAYS initialize UI Event Listeners FIRST so buttons (Realistic/Visual, Play, Speed) ALWAYS work 100%!
        initNavTabs();
        initQuickBar();
        initAtlasGrid();
        initSpaceCalc();
        ...
        function loadPlanetTexture(key) {
            if (LOCAL_REAL_TEXTURES[key] && typeof THREE !== 'undefined' && THREE.TextureLoader) {
                try {
                    ...
                    var tex = loader.load(LOCAL_REAL_TEXTURES[key]);
                    if (tex) {
