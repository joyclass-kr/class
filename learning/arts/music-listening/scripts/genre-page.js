...
function renderCards(lineage='all'){
  ...
  playlist.innerHTML=cards.map((card,index)=>`<article class="piece"><p class="meta">${card.years} · ${card.style} (${card.styleEn})</p><h3>${card.title}</h3><p class="original">${card.original}</p><p class="artist">${card.artist}</p><p class="point">${card.point}</p><div class="actions"><a href="https://www.youtube.com/results?search_query=${encodeURIComponent(card.artist+' '+card.original)}" target="_blank" rel="noopener">찾아 듣기</a><button data-card="${data.cards.indexOf(card)}">자세한 해설</button></div></article>`).join('');
...
function openDetail(index){
  ...
  detail.innerHTML=`<p class="kind">${card.years} · ${card.style} (${card.styleEn})</p><h2>${card.title}</h2><p class="original">${card.original} · ${card.artist}</p><section><h3>음악사 속 위치</h3><p>${card.history}</p></section><section><h3>음악적 특징</h3><p>${card.sound}</p></section><section><h3>감상 포인트</h3><p>${card.point}</p></section>`;
  dialog.showModal();
}
