...
def write_glb(path: Path, positions: np.ndarray, normals: np.ndarray, indices: np.ndarray):
    ...
    doc = {
        "asset": {"version": "2.0", "generator": "Detailed Museum 3D Scan Pipeline"},
        "scene": 0,
        "scenes": [{"nodes": [0]}],
        "nodes": [{"mesh": 0}],
        "meshes": [{"primitives": [{
    ...
    print(f"Detailed 3D Scan GLB: {path.name} ({len(glb)} bytes, {len(positions)} verts, {len(indices)//3} tris)")
...
class MeshBuilder:
    ...
    def add_triangle(self, p1, p2, p3, n1=None, n2=None, n3=None):
        if n1 is None:
            v1, v2, v3 = np.array(p1), np.array(p2), np.array(p3)
            normal = np.cross(v2 - v1, v3 - v1)
            norm_val = np.linalg.norm(normal)
            normal = normal / norm_val if norm_val > 1e-6 else np.array([0.0, 1.0, 0.0])
            n1 = n2 = n3 = normal.tolist()
        idx = len(self.verts)
        self.verts.extend([p1, p2, p3])
        self.norms.extend([n1, n2, n3])
        self.idxs.extend([idx, idx + 1, idx + 2])
    ...
    def add_cylinder(self, bottom_center, r_top, r_bottom, height, segments=24):
...
# 1. Detailed Dabotap & Seokgatap
b_towers = MeshBuilder()
# Seokgatap (3-Story Pagoda)
b_towers.add_box([-5.0, 0.6, 0], [4.5, 1.2, 4.5])
b_towers.add_box([-5.0, 1.8, 0], [3.5, 1.2, 3.5])
b_towers.add_box([-5.0, 2.9, 0], [2.5, 1.0, 2.5])
b_towers.add_box([-5.0, 3.7, 0], [3.8, 0.6, 3.8]) # 1st Roof
b_towers.add_box([-5.0, 4.5, 0], [1.9, 0.9, 1.9])
b_towers.add_box([-5.0, 5.2, 0], [3.0, 0.5, 3.0]) # 2nd Roof
b_towers.add_box([-5.0, 6.0, 0], [1.4, 0.8, 1.4])
b_towers.add_box([-5.0, 6.6, 0], [2.2, 0.4, 2.2]) # 3rd Roof
b_towers.add_cylinder([-5.0, 6.8, 0], 0.1, 0.4, 3.8, segments=16) # Finial

# Dabotap (Elaborate Pagoda)
b_towers.add_box([5.0, 0.6, 0], [4.5, 1.2, 4.5]) # Base
for px in [3.6, 6.4]:
    for pz in [-1.4, 1.4]:
        b_towers.add_box([px, 1.8, pz], [0.6, 1.2, 0.6]) # 4 Corner Pillars
b_towers.add_box([5.0, 2.7, 0], [3.8, 0.6, 3.8]) # Roof Balustrade
b_towers.add_cylinder([5.0, 3.3, 0], 1.4, 1.4, 1.2, segments=8) # Octagonal Body
b_towers.add_box([5.0, 4.5, 0], [3.2, 0.5, 3.2]) # Bamboo Eaves
b_towers.add_cylinder([5.0, 5.2, 0], 0.8, 1.2, 1.2, segments=16) # Lotus Crown
b_towers.add_cylinder([5.0, 6.4, 0], 0.1, 0.4, 3.8, segments=16) # Finial
...
b_liberty.add_cylinder([0, 48, 0], 3.2, 4.8, 26, segments=24)
...
print("ALL DETAILED 3D MODELS BUILT SUCCESSFULLY!")
