...
# 2x supersampling for high sharpness: 1200x1600 -> 600x800
W, H = 1200, 1600
...
font_back_title = get_font(88, bold=True)
...
def create_base_card(category_type, title_text, tagline_text, bg_color1, bg_color2):
    ...
    # Corner stars
    draw_star(draw, inner_margin + 28, inner_margin + 28, 20, 9, fill=(255, 215, 0, 255))
    draw_star(draw, W - inner_margin - 28, inner_margin + 28, 20, 9, fill=(255, 215, 0, 255))
    draw_star(draw, inner_margin + 28, H - inner_margin - 28, 20, 9, fill=(255, 215, 0, 255))
    draw_star(draw, W - inner_margin - 28, H - inner_margin - 28, 20, 9, fill=(255, 215, 0, 255))
    ...
    draw.rounded_rectangle([badge_x, badge_y, badge_x + badge_w, badge_y + badge_h],
                           radius=38, fill=badge_bg, outline=(255, 255, 255, 255), width=6)
    ...
    draw.polygon([(tx - 45, title_y + th//2), (tx - 30, title_y + th//2 - 15), (tx - 15, title_y + th//2), (tx - 30, title_y + th//2 + 15)], fill=accent_color)
    draw.polygon([(tx + tw + 15, title_y + th//2), (tx + tw + 30, title_y + th//2 - 15), (tx + tw + 45, title_y + th//2), (tx + tw + 30, title_y + th//2 + 15)], fill=accent_color)

    # Main Illustration Frame
    frame_x1, frame_y1 = margin + 50, title_y + th + 36
    ...
    tag_y = frame_y2 + 30
    tag_w, tag_h = W - (margin + 100) * 2, 72
    tag_x = (W - tag_w) // 2
    draw.rounded_rectangle([tag_x, tag_y, tag_x + tag_w, tag_y + tag_h], radius=36, fill=badge_bg, outline=(255, 255, 255, 255), width=4)
...
# --- 10 CARDS GENERATOR FUNCTIONS ---

# 1. Dancer (무희)
def make_dancer():
    img, (fx, fy, fw, fh) = create_base_card("suspect", "무희", "무대를 사로잡는 무희", (255, 170, 160), (210, 30, 70))
    draw = ImageDraw.Draw(img)
    cx, cy = fx + fw // 2, fy + fh // 2

    # Floating Musical Notes & Sparkles
    draw_sparkle(draw, fx + 80, fy + 80, 26)
    draw_sparkle(draw, fx + fw - 80, fy + 90, 26)
    draw_sparkle(draw, fx + 100, fy + fh - 100, 20)

    # Hair
    draw.ellipse([cx - 180, cy - 230, cx + 180, cy + 150], fill=(170, 35, 60), outline=(80, 15, 30), width=5)
    # Feather Headband
    draw.polygon([(cx - 70, cy - 180), (cx - 110, cy - 290), (cx - 20, cy - 200)], fill=(255, 215, 0), outline=(180, 140, 0), width=4)
    # Face
    draw.ellipse([cx - 130, cy - 150, cx + 130, cy + 90], fill=(255, 225, 210), outline=(160, 100, 80), width=5)
    # Blush
    draw.ellipse([cx - 105, cy + 10, cx - 55, cy + 45], fill=(255, 140, 160, 180))
    draw.ellipse([cx + 55, cy + 10, cx + 105, cy + 45], fill=(255, 140, 160, 180))
    # Eyes & Smile
    ...
    # Dancing Dress
    draw.polygon([(cx - 180, cy + 290), (cx - 80, cy + 80), (cx + 80, cy + 80), (cx + 180, cy + 290)], fill=(255, 69, 0), outline=(180, 30, 0), width=5)
...
def make_kitchen():
    ...
    # Chef Hat Icon
    draw.ellipse([cx - 90, cy - 220, cx + 90, cy - 110], fill=(255, 255, 255), outline=(200, 200, 220), width=4)
    draw.rectangle([cx - 60, cy - 140, cx + 60, cy - 100], fill=(255, 255, 255), outline=(200, 200, 220), width=4)
...
def make_hall():
    ...
    # Brass Door Handle
    draw.ellipse([cx + 50, cy + 20, cx + 80, cy + 50], fill=(255, 215, 0), outline=(180, 140, 0), width=4)

    # Red Carpet
    draw.polygon([(cx - 80, cy + 190), (cx + 80, cy + 190), (cx + 150, cy + 240), (cx - 150, cy + 240)], fill=(211, 47, 47), outline=(255, 215, 0), width=4)
...
def main():
    print("Generating all 10 cards cleanly without any composite artifacts...")
    make_dancer()
    make_candlestick()
    make_rope()
    make_kitchen()
