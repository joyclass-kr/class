...
function complete(){if(state.locked)return;state.locked=true;const current=target(),given=input.value;if(given!==current){input.classList.add('error');feedback.className='feedback bad';feedback.textContent='빨간 글자를 고쳐 보세요.';ping(false);state.locked=false;return}const result=stats();state.streak++;state.speeds.push(result.speed);feedback.className='feedback good';feedback.textContent='정확해요! 참 잘했어요.';ping(true);setTimeout(()=>{state.index++;state.locked=false;if(state.index>=state.items.length)finish();else showItem()},420)}
...
input.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();input.value===target()?complete():skip();return}if(e.ctrlKey||e.altKey||e.metaKey)return;if(e.key==='Backspace'){e.preventDefault();forcedKeyProgress=Math.max(0,forcedKeyProgress-1);input.value=completedTargetPrefix(forcedKeyProgress);input.classList.remove('error');renderPrompt();return}const pressed=physicalCharacter(e);if(!pressed)return;e.preventDefault();if(!state.start)state.start=Date.now();state.typed++;const expected=targetKeySequence()[forcedKeyProgress];if(pressed===expected){state.correct++;forcedKeyProgress++;input.value=completedTargetPrefix(forcedKeyProgress);input.classList.remove('error');feedback.className='feedback';feedback.textContent='좋아요. 그대로 이어서 입력하세요!';renderPrompt();stats();if(forcedKeyProgress===targetKeySequence().length)complete()}else{input.classList.add('error');feedback.className='feedback bad';feedback.textContent=`다른 키예요. ${state.lang==='ko'?'한글':state.lang==='en'?'영어':'숫자·기호'} 키 위치대로 눌러 보세요.`;renderPrompt();stats();ping(false)}});
