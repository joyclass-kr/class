...
const paragraphPools={ko:[...lessons.ko[2]],en:[...lessons.en[2]]};
...
function setup(){const source=state.mode==='words'?wordPools[state.lang]:state.mode==='sentences'?sentencePools[state.lang]:paragraphPools[state.lang];state.items=choosePracticeItems(source);state.index=0;state.start=0;state.typed=0;state.correct=0;state.streak=0;state.speeds=[];state.locked=false;input.value='';input.disabled=false;$('.practice').hidden=false;$('#resultCard').hidden=true;showItem();stats();setTimeout(()=>input.focus(),50)}
