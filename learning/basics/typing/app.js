...
async function loadWordPools(){try{const [vocabulary,...koreanResponses]=await Promise.all([fetch('/learning/basics/vocabulary/assets/data/english-vocabulary-3000-v2.json').then(response=>response.json()),...['/learning/basics/spelling/questions.js','/learning/basics/spelling/questions-extra.js','/game-hub-server/data/reading-self-study-v2.js','/game-hub-server/data/reading-independent-topics-v3.js'].map(path=>fetch(path).then(response=>response.text()))]);const english=vocabulary.words.filter(item=>item.stageCode==='elementary'&&/^[a-zA-Z]+$/.test(item.word)&&item.word.length>=2&&item.word.length<=12).map(item=>item.word.toLowerCase());const englishSentences=vocabulary.words.filter(item=>item.stageCode==='elementary'&&item.example?.en).map(item=>item.example.en.toLowerCase().replace(/!/g,'.')).filter(sentence=>sentence.length>=15&&sentence.length<=100&&/^[a-z0-9 ,.'?-]+$/.test(sentence));wordPools.en=[...new Set(english)];wordPools.ko=[...new Set(koreanResponses.flatMap(collectKoreanWords))].filter(word=>word.length<=5);sentencePools.ko=[...new Set(koreanResponses.flatMap(collectKoreanSentences))];sentencePools.en=[...new Set(englishSentences)];return true}catch(error){console.warn('학습 자료를 불러오지 못해 기본 자료를 사용합니다.',error);return false}}
...
function choosePracticeItems(source){if(state.mode==='paragraphs')return pickRandom(source,10);const recentKey=`typing-recent-${state.mode}-${state.lang}`;let recent=[];try{recent=JSON.parse(localStorage.getItem(recentKey)||'[]')}catch{}const fresh=source.filter(item=>!recent.includes(item)),chosen=pickRandom(fresh.length>=10?fresh:source,10);localStorage.setItem(recentKey,JSON.stringify(chosen));return chosen}
function setup(){const source=state.mode==='words'?wordPools[state.lang]:state.mode==='sentences'?sentencePools[state.lang]:lessons[state.lang][state.level];state.items=choosePracticeItems(source);state.index=0;state.start=0;state.typed=0;state.correct=0;state.streak=0;state.speeds=[];state.locked=false;input.value='';input.disabled=false;$('.practice').hidden=false;$('#resultCard').hidden=true;showItem();stats();setTimeout(()=>input.focus(),50)}
function showItem(){input.value='';resetForcedInput();input.classList.remove('error');feedback.className='feedback';feedback.textContent='한/영 키를 바꾸지 않아도 바로 입력할 수 있어요.';$('#stepLabel').textContent=`${state.index+1} / 10`;$('#bestLabel').textContent=`최고 ${localStorage.getItem(key())||0}타/분`;$('#progressBar').style.width=`${(state.index+1)*10}%`;renderPrompt()}
