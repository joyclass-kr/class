...
let forcedKeyProgress=0,previousInputLength=0,audioContext=null;
...
input.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();playKeySound(true);input.value===target()?complete():skip();return}if(e.ctrlKey||e.altKey||e.metaKey)return;if(e.key==='Backspace'){e.preventDefault();playKeySound(true);forcedKeyProgress=Math.max(0,forcedKeyProgress-1);input.value=completedTargetPrefix(forcedKeyProgress);previousInputLength=chars(input.value).length;input.classList.remove('error');renderPrompt();return}const pressed=physicalCharacter(e);if(!pressed)return;e.preventDefault();playKeySound();if(!state.start)state.start=Date.now();state.typed++;const expected=targetKeySequence()[forcedKeyProgress];if(pressed===expected){state.correct++;forcedKeyProgress++;input.value=completedTargetPrefix(forcedKeyProgress);previousInputLength=chars(input.value).length;input.classList.remove('error');feedback.className='feedback';feedback.textContent='좋아요. 그대로 이어서 입력하세요!';renderPrompt();stats();if(forcedKeyProgress===targetKeySequence().length)complete()}else{input.classList.add('error');feedback.className='feedback bad';feedback.textContent=`다른 키예요. ${state.lang==='ko'?'한글':state.lang==='en'?'영어':'숫자·기호'} 키 위치대로 눌러 보세요.`;renderPrompt();stats();playErrorSound()}});
