...
const state={lang:'ko',level:0,mode:'words',filter:'easy',items:[],index:0,start:0,typed:0,correct:0,totalTyped:0,totalCorrect:0,streak:0,speeds:[],activeMs:0,sound:true,sfxVolume:.6,locked:false};
...
function renderPracticeFilters(){const options=state.lang==='avatars'?[['animal','동물'],['food','과일·채소·음식'],['object','사물']]:[['easy','쉬움'],['normal','보통'],['hard','어려움']];$('#practiceFilters').innerHTML=options.map(([value,label])=>`<button type="button" class="${state.filter===value?'active':''}" data-practice-filter="${value}" aria-pressed="${state.filter===value}">${label}</button>`).join('');$$('[data-practice-filter]').forEach(button=>button.addEventListener('click',()=>{state.filter=button.dataset.practiceFilter;renderPracticeFilters();setup()}))}
function wordDifficulty(word){if(state.lang==='en')return word.length<=4?'easy':word.length<=7?'normal':'hard';if(state.lang==='symbols')return word.length<=5?'easy':word.length<=9?'normal':'hard';const syllables=[...word].length;return syllables<=2?'easy':syllables===3?'normal':'hard'}
