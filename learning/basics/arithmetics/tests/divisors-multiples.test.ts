...
test("한 수가 다른 수의 정확히 2배인 수쌍은 한 페이지에 최대 한 번만 나온다", () => {
  for (let seed = 20260721; seed < 20260741; seed += 1) {
    const problems = createDivisorMultipleSet(seed).columns.flat();
    const doubledPairs = problems.filter(({ left, right }) => (
      Math.max(left, right) === Math.min(left, right) * 2
    ));
    assert.ok(doubledPairs.length <= 1, "seed " + seed + ": " + doubledPairs.length + "개");
  }
});
