...
test("중등 54개 목차는 중복 없이 모두 실제 페이지에 연결된다", () => {
  ...
  assert.equal(new Set(middleSchoolWorksheetCatalog.map(({ name }) => name)).size, 54);
  assert.equal(new Set(middleSchoolWorksheetCatalog.map(({ route }) => route)).size, 54);
  assert.deepEqual(
