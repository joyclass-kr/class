...
test("원의 둘레와 넓이는 서로 다른 복합도형 여덟 문제를 만든다", () => {
  const problems = createGradeSixCircleSet(20260722);
  assert.equal(problems.length, 8);
  assert.equal(new Set(problems.map((problem) => problem.kind)).size, 8);
...
test("고리 모양의 둘레는 안쪽 원의 둘레까지 포함한다", () => {
  const problem = createGradeSixCircleSet(20260722).find((item) => item.kind === "annulus");
  assert.ok(problem);
